...
func TestTakeOneFromSlot(t *testing.T) {
	...
	assert.True(t, inventory.TakeOneFromSlot(0), "Should take one from slot when empty dragged")
	...
	assert.True(t, inventory.TakeOneFromSlot(0), "Should stack onto dragged slot")
	...
	assert.False(t, inventory.TakeOneFromSlot(0), "Should not take when dragged slot is full")

	inventory.Slots[0].Quantity = 1
	draggedSlot.Item.Quantity = draggedSlot.Item.MaxStackSize - 1
	assert.True(t, inventory.TakeOneFromSlot(0), "Should take last item when dragged")
	...
	assert.False(t, inventory.TakeOneFromSlot(1), "Should not take when dragged slot has different item")

	// Non-stackable item in slot
	inventory.Slots[2] = &Item{ID: 3, Quantity: 1, Stackable: false, MaxStackSize: 1}
	draggedSlot.Empty = true
	assert.False(t, inventory.TakeOneFromSlot(2), "Should not take when item is not stackable")

	// Out of bounds index
	assert.False(t, inventory.TakeOneFromSlot(-1), "Should not take for invalid slot index")
	assert.False(t, inventory.TakeOneFromSlot(len(inventory.Slots)), "Should not take for invalid slot index")
}
...
func TestDropToSlot(t *testing.T) {
	...
	assert.True(t, inventory.DropToSlot(0))
	...
	assert.True(t, inventory.DropToSlot(1))
	...
	assert.True(t, inventory.DropToSlot(2))
...
func TestPickUpFromSlot(t *testing.T) {
	...
	assert.True(t, inventory.PickUpFromSlot(0))
	...
	assert.False(t, inventory.PickUpFromSlot(0))
	...
	assert.True(t, inventory.PickUpFromSlot(1))
